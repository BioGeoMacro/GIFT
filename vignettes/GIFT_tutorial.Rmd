...
ggplot(world) +
  ...
  geom_sf(data = med, fill = "darkblue", color = "black", alpha = 0.8) +
...
<br>
Use of `GIFT_references()` for citation and geo_entity_ref.
...
To know what species are available, you can run the function `GIFT_species()`.

```{r}
all_sp <- GIFT_species()
```

`r length(unique(all_sp$work_ID))` species are currently available.

## 2.2. Species names and taxonomic harmonization
The column `work_species` is listing all species available
**after taxonomic harmonization**. This is important to consider as species
names may be stated differently in the original sources.
<br>
To have an overview of all the original species names were in the checklists,
you can run the function `GIFT_species_lookup()`.
<br>
```{r fagus_lookup}
fagus_lookup <- GIFT_species_lookup(genus = "Fagus", epithet = "sylvatica")
knitr::kable(fagus_lookup)
```

Looking at the output table, you can see the original species names and their
identification numbers (`name_ID`) before taxonomic harmonization. The final
names and ID are the last column on the right starting with `work_`.

<br><br>

## 2.3. Species distribution
<br>
Now that we have a species name, we can run the function
`GIFT_species_distribution()` to know where it occurs.
<br>
```{r}
fagus_distr <- GIFT_species_distribution(genus = "Fagus",
                                         epithet = "sylvatica")
```
...
gift_shape <- GIFT_shape(entity_ID = fern_height$entity_ID)
...
Mediterranean basin, retrieved in the section 1.
<br>
...
the GIFT database.
<br>
The references to cite when using environemental variables are also available
