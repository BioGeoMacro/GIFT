...
```{r}
phy <- suppressMessages(GIFT_phylogeny(clade = "Tracheophyta",
                                       GIFT_version = "beta"))
tax <- suppressMessages(GIFT_taxonomy(GIFT_version = "beta"))
gift_sp <- suppressMessages(GIFT_species(GIFT_version = "beta"))

gf <- suppressMessages(
  GIFT_traits(trait_IDs = "1.2.1",
              agreement = 0.66,
              bias_ref = FALSE, bias_deriv = FALSE,
              GIFT_version = "beta"))
...
for(i in 1:n_distinct(sp_genus_fam$genus)){ # loop over genera
  ...
  print(paste0(i, "/", n_distinct(sp_genus_fam$genus)))
}

# Adding the trait coverage per genus
one_sp_per_gen <- left_join(one_sp_per_gen, genus_gf, by = "genus")

# Ten most frequent families
sort(table(one_sp_per_gen$family), decreasing = TRUE)[1:10]
...
length(one_sp_per_gen[complete.cases(one_sp_per_gen$species), "species"]) # 15396
