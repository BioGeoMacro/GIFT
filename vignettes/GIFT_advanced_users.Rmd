...
## 5.2. By ref_ID
In `GIFT_checklist()`, there is also the possibility to remove overlapping
polygons only if they belong to the same reference (i.e. same `ref_ID`).  
<br>
We show how this works with the following example:  

```{r}
ex <- GIFT_checklist(taxon_name = "Tracheophyta", by_ref_ID = FALSE,
                     list_set_only = TRUE)
ex2 <- GIFT_checklist(taxon_name = "Tracheophyta",
                      remove_overlap = TRUE, by_ref_ID = TRUE,
                      list_set_only = TRUE)
ex3 <- GIFT_checklist(taxon_name = "Tracheophyta",
...
pilbara <- GIFT_shape(entity_ID = c(10043, 12172, 11398, 11391, 10918))
