...
25 references are in the second example and not in the third. If we look at one
of the listed references `ref_ID = 10143`, we see that it is a checklist for
the Pilbara region in Australia. Its `entity_ID` is 10043. Looking at the GIFT
web site, we see that other regions can overlap with it.  
<br>
...
`by_ref_ID = TRUE` but are ignored if `by_ref_ID = FALSE`.
