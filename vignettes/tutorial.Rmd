...
We here retrieve woodiness and life information for every species using `DB_get_traits()` function.
...
```{r get_traits}
ex_tra <- DB_get_traits(trait_IDs = c("1.1.1", "1.2.1"), agreement = 0.66,
...
# 3. Metadata
## 3.1. Environment
The functions `env_raster()` and `env_misc()` allow the user to retrieve metadata about environmental variables.
...
## 3.2. Traits
The function `traits_meta()` allows the user to retrieve metadata about available traits.
...
## 3.3. References
The function `references()` allows the user to retrieve metadata about the references that were used to build GIFT database.

```{r traits_meta}
ref <- references(restricted = FALSE)
head(ref)
