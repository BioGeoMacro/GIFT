...
test_that("length of format inferior or equal to length of output",
          {
            ...
            expect_length(ncol(ex), c(2L))
            
            ex2 <- GIFT_no_overlap(entity_IDs = c(10071, 12078))
            expect_length(ncol(ex2), c(1L))
...
expect_error(
  ...
  "Please provide the ID numbers of the regions you want to check the overlap
  of.", fixed = TRUE)

expect_error(
  GIFT_no_overlap(entity_IDs = c(1, 2), area_th_island = NULL),
  "'area_th_island' is a surface in km^2 indicating from which surface the
  smallest overlapping polygon is kept.", fixed = TRUE)

expect_error(
  GIFT_no_overlap(entity_IDs = c(1, 2), area_th_mainland = NULL),
  "'area_th_mainland' is a surface in km^2 indicating from which surface the
  smallest overlapping polygon is kept.", fixed = TRUE)
